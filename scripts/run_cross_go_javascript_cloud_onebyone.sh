...
ARTIFACT_DIR="${LCP_CROSS_GO_JS_CLOUD_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/cross-go-javascript-cloud-onebyone-$(date +%Y%m%d-%H%M%S)}"
...
GO_CASES=(
  "TestPersistentStoreCloudContractSetGetValue"
  "TestPersistentStoreCloudContractOverwrite"
  "TestPersistentStoreCloudContractDelete"
  "TestPersistentStoreCloudContractClear"
  "TestPersistentStoreCloudContractPruneExpired"
  "TestPersistentStoreCloudContractHydrateAllValidAndLimit"
)
...
export GOOGLE_CLOUD_PROJECT="${GOOGLE_CLOUD_PROJECT:-aiptesting}"
export GCP_SA_KEY="${GCP_SA_KEY:-$ROOT_DIR/sdks/go/config/testing/aiptesting-firebase-adminsdk-fbsvc-398b4932fd.json}"
export LCP_CROSS_GO_CLOUD_STORAGE_URI="${LCP_CROSS_GO_CLOUD_STORAGE_URI:-gs://aiptesting.firebasestorage.app/lcp}"

echo "[1/5] Preparing artifact dir: $ARTIFACT_DIR" | tee "$RUN_LOG"
echo "Using cloud URI: $LCP_CROSS_GO_CLOUD_STORAGE_URI" | tee -a "$RUN_LOG"
...
echo "[3/5] Running Go one-by-one cases (memory + cloud-storage in each case)" | tee -a "$RUN_LOG"
...
const report = {
  ...
  baseline: 'javascript',
  go_backends: ['memory', 'cloud-storage'],
  js_backend: 'indexeddb',
  cases
};
